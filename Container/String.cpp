...
void FString::Replace(char replaceThis, char replaceWith, bool caseSensitive)
{
    ...
    {
        replaceThis = KhFileSystem::ToLower(replaceThis);
        for (Iterator it = Begin(); it != End(); ++it)
        {
            if (KhFileSystem::ToLower(*it) == replaceThis)
                *it = replaceWith;
        }
    }
}
...
{
    KhFileSystem::Swap(_buffer, str._buffer);
}
...
size_t FString::Find(char c, size_t startPos, bool caseSensitive) const
{
    ...
    {
        c = KhFileSystem::ToLower(c);
        for (size_t i = startPos; i < Length(); ++i)
        {
            if (KhFileSystem::ToLower(Buffer()[i]) == c)
                return i;
        }
    }
...
size_t FString::Find(const FString& str, size_t startPos, bool caseSensitive) const
{
    ...
    if (!caseSensitive)
        first = KhFileSystem::ToLower(first);
...
unsigned FString::CaseInsensitiveHash(const char* str)
{
    ...
    while (*str)
    {
        hash = (KhFileSystem::ToLower(*str)) + (hash << 6) + (hash << 16) - hash;
        ++str;
    }
...
bool FString::ToBool(const char* str)
{
    ...
    {
        char c = KhFileSystem::ToLower(str[i]);
        if (c == 't' || c == 'y' || c == '1')
            return true;
        else if (c != ' ' && c != '\t')
            break;
...
int FString::Compare(const char* lhs, const char* rhs, bool caseSensitive)
{
    ...
    else
    {
        for (;;)
        {
            char l = KhFileSystem::ToLower(*lhs);
            char r = KhFileSystem::ToLower(*rhs);
            if (!l || !r)
                return l ? 1 : (r ? -1 : 0);
            if (l < r)
                return -1;
